...
如果是正常执行，在回调函数中，我们首先要做幂等性检查，根据订单支付的状态来判断是否支付成功，避免重复更新；如果没成功，那么通过传入的交易流水号查询交易流水表中是否存在这条记录（这一步是为了防伪操作），然后通过相关业务操作最终更改里面的回调状态（true，表示已触发）和订单支付表的状态
...
**RDB是二进制文件，在保存的时候体积也比较小（`保存的是某一时刻内存数据`），恢复的比较快，但是可能会丢数据；A0F恢复速度慢一些（`记录的是每次写操作`），但是它丢数据的风险要小很多**，可以设置A0F的刷盘策略，以每秒批量写入命令
